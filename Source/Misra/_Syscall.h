...
static inline long misra_sys1(long nr, long a) {
    long ret;
#    if defined(__x86_64__)
    __asm__ volatile("syscall" : "=a"(ret) : "0"(nr), "D"(a) : "rcx", "r11", "memory");
#    else
    register long x8 __asm__("x8") = nr;
...
static inline long misra_sys2(long nr, long a, long b) {
    long ret;
#    if defined(__x86_64__)
    __asm__ volatile("syscall" : "=a"(ret) : "0"(nr), "D"(a), "S"(b) : "rcx", "r11", "memory");
#    else
    register long x8 __asm__("x8") = nr;
    register long x0 __asm__("x0") = a;
...
static inline long misra_sys3(long nr, long a, long b, long c) {
    long ret;
#    if defined(__x86_64__)
    __asm__ volatile("syscall" : "=a"(ret) : "0"(nr), "D"(a), "S"(b), "d"(c) : "rcx", "r11", "memory");
#    else
    register long x8 __asm__("x8") = nr;
    register long x0 __asm__("x0") = a;
    register long x1 __asm__("x1") = b;
...
#    if defined(__x86_64__)
    ...
    __asm__ volatile("syscall" : "=a"(ret) : "0"(nr), "D"(a), "S"(b), "d"(c), "r"(r10) : "rcx", "r11", "memory");
#    else
    register long x8 __asm__("x8") = nr;
    register long x0 __asm__("x0") = a;
    register long x1 __asm__("x1") = b;
