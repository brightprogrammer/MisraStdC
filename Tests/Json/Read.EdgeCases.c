...
bool test_empty_string_reading(void) {
    ...
    if (obj.name.length == 0 && obj.description.length == 0) {
        ...
        printf("[DEBUG] Empty string test FAILED - name len: %zu, desc len: %zu\n", 
               obj.name.length, obj.description.length);
        success = false;
    }
...
bool test_negative_numbers_reading(void) {
    ...
    if (obj.temp == -25 && obj.balance == -1000.50 && obj.delta == -0.001) {
        printf("[DEBUG] Negative numbers test passed - temp: %d, balance: %f, delta: %f\n", 
               obj.temp, obj.balance, obj.delta);
    } else {
        printf("[DEBUG] Negative numbers test FAILED - temp: %d, balance: %f, delta: %f\n", 
               obj.temp, obj.balance, obj.delta);
        success = false;
    }
...
bool test_large_numbers_reading(void) {
    ...
    Str json = StrInitFromZstr("{\"big_int\":9223372036854775807,\"big_float\":1.7976931348623157e+308,\"small_float\":2.2250738585072014e-308}");
    StrIter si = StrIterFromStr(json);
...
bool test_zero_values_reading(void) {
    ...
    if (obj.int_zero == 0 && obj.float_zero == 0.0 && obj.bool_false == false) {
        printf("[DEBUG] Zero values test passed - int: %d, float: %f, bool: %s\n", 
               obj.int_zero, obj.float_zero, obj.bool_false ? "true" : "false");
    } else {
        printf("[DEBUG] Zero values test FAILED - int: %d, float: %f, bool: %s\n", 
               obj.int_zero, obj.float_zero, obj.bool_false ? "true" : "false");
        success = false;
    }
...
bool test_special_characters_in_strings(void) {
    ...
    Str json = StrInitFromZstr("{\"path\":\"C:\\\\Program Files\\\\App\",\"message\":\"Hello, \\\"World\\\"!\",\"data\":\"line1\\nline2\\ttab\"}");
    StrIter si = StrIterFromStr(json);
...
bool test_escape_sequences_reading(void) {
    ...
    Str json = StrInitFromZstr("{\"escaped\":\"\\\"quotes\\\"\",\"backslash\":\"\\\\\",\"newline\":\"\\n\",\"tab\":\"\\t\"}");
    StrIter si = StrIterFromStr(json);
    ...
    if (obj.escaped.length > 0 && obj.backslash.length > 0 && 
        obj.newline.length > 0 && obj.tab.length > 0) {
...
bool test_whitespace_variations_reading(void) {
    ...
    if (StrCmpCstr(&obj.name, "test", 4) == 0 && obj.value == 42 && obj.flag == true) {
        printf("[DEBUG] Whitespace variations test passed - name: %s, value: %d, flag: %s\n", 
               obj.name.data, obj.value, obj.flag ? "true" : "false");
    } else {
        printf("[DEBUG] Whitespace variations test FAILED - name: %s, value: %d, flag: %s\n", 
               obj.name.data, obj.value, obj.flag ? "true" : "false");
        success = false;
    }
...
bool test_nested_empty_containers(void) {
    ...
    if (!obj.found_outer && !obj.found_list && obj.found_deep && !obj.found_inner) {
        ...
        printf("[DEBUG] Nested empty containers test results - outer: %s, list: %s, deep: %s, inner: %s\n",
               obj.found_outer ? "true" : "false", obj.found_list ? "true" : "false",
               obj.found_deep ? "true" : "false", obj.found_inner ? "true" : "false");
    }
...
bool test_mixed_empty_and_filled(void) {
    ...
    JR_OBJ(si, {
        JR_OBJ_KV(si, "empty_obj", {
            // Should not find anything
        });
        JR_OBJ_KV(si, "filled_obj", {
            JR_INT_KV(si, "x", obj.x_value);
        });
        JR_ARR_KV(si, "empty_arr", {
    ...
    if (obj.x_value == 1 && VecLen(&obj.filled_items) == 2 && 
        VecAt(&obj.filled_items, 0) == 1 && VecAt(&obj.filled_items, 1) == 2) {
        printf("[DEBUG] Mixed empty and filled test passed - x: %d, items: %zu\n", 
               obj.x_value, VecLen(&obj.filled_items));
    } else {
        printf("[DEBUG] Mixed empty and filled test FAILED - x: %d, items: %zu\n", 
               obj.x_value, VecLen(&obj.filled_items));
...
bool test_boundary_integers(void) {
    ...
    if (obj.max_int == 2147483647LL && obj.min_int == -2147483648LL && 
        obj.one == 1 && obj.minus_one == -1) {
        printf("[DEBUG] Boundary integers test passed - max: %lld, min: %lld, one: %lld, minus_one: %lld\n", 
               obj.max_int, obj.min_int, obj.one, obj.minus_one);
    } else {
        printf("[DEBUG] Boundary integers test FAILED - max: %lld, min: %lld, one: %lld, minus_one: %lld\n", 
               obj.max_int, obj.min_int, obj.one, obj.minus_one);
        success = false;
    }
...
bool test_boundary_floats(void) {
    ...
    if (obj.tiny > 0.0000001 && obj.tiny < 0.00001 && 
        obj.huge > 999999.0 && obj.huge < 1000000.0 &&
        obj.zero == 0.0 && obj.negative_tiny < -0.0000001 && obj.negative_tiny > -0.00001) {
        printf("[DEBUG] Boundary floats test passed - tiny: %f, huge: %f, zero: %f, neg_tiny: %f\n", 
               obj.tiny, obj.huge, obj.zero, obj.negative_tiny);
    } else {
        printf("[DEBUG] Boundary floats test FAILED - tiny: %f, huge: %f, zero: %f, neg_tiny: %f\n", 
               obj.tiny, obj.huge, obj.zero, obj.negative_tiny);
        success = false;
    }
...
int main(int argc, char* argv[]) {
    ...
    return run_test_suite(tests, test_count,
                         NULL, 0,
                         "Json.Read.EdgeCases");
} 