...
bool test_simple_nested_object_writing(void) {
    ...
    struct {
        ...
        bool active;
    } data = {{StrInitFromZstr("Charlie"), StrInitFromZstr("charlie@example.com")}, true};
...
bool test_simple_product_with_tags_writing(void) {
    ...
    JW_OBJ(json, {
        ...
        JW_ARR_KV(json, "tags", product.tags, tag, {
            JW_STR(json, tag);
        });
    });
    
    const char* expected = "{\"id\":12345,\"name\":\"Laptop\",\"price\":999.990000,\"tags\":[\"electronics\",\"computers\",\"portable\"]}";
    if (!compare_json_output(&json, expected)) {
        success = false;
    }
