...
bool test_two_level_nesting(void) {
    ...
    bool success = true;
    Str json = StrInitFromZstr("{\"user\": {\"id\": 123, \"profile\": {\"name\": \"Alice\", \"age\": 30}}, \"status\": \"active\"}");
    StrIter si = StrIterFromStr(json);
...
bool test_three_level_nesting(void) {
    ...
    Str json = StrInitFromZstr(
        "{\"company\": {\"departments\": {\"engineering\": {\"head\": \"John\", \"count\": 25, \"budget\": 150000.0}}, \"name\": \"TechCorp\"}}"
    );
    StrIter si = StrIterFromStr(json);
...
bool test_dynamic_key_parsing(void) {
    ...
    bool success = true;
    Str json = StrInitFromZstr(
        "{\"functions\": {\"12345\": {\"67890\": {\"distance\": 0.85, \"name\": \"main\"}}, \"54321\": {\"98765\": {\"distance\": 0.92, \"name\": \"helper\"}}}}"
    );
    StrIter si = StrIterFromStr(json);
    ...
    if (symbols.length >= 2) {
        ...
        if (!(sym1->source_function_id == 12345 || sym1->source_function_id == 54321)) {
            printf("[DEBUG] Sym1 source function ID check failed: expected 12345 or 54321, got %llu\n", sym1->source_function_id);
            success = false;
        }
        
        if (!(sym1->target_function_id == 67890 || sym1->target_function_id == 98765)) {
            printf("[DEBUG] Sym1 target function ID check failed: expected 67890 or 98765, got %llu\n", sym1->target_function_id);
            success = false;
        }
        ...
        if (!(sym2->source_function_id == 12345 || sym2->source_function_id == 54321)) {
            printf("[DEBUG] Sym2 source function ID check failed: expected 12345 or 54321, got %llu\n", sym2->source_function_id);
            success = false;
        }
        
        if (!(sym2->target_function_id == 67890 || sym2->target_function_id == 98765)) {
            printf("[DEBUG] Sym2 target function ID check failed: expected 67890 or 98765, got %llu\n", sym2->target_function_id);
            success = false;
        }
...
bool test_complex_api_response(void) {
    ...
    bool success = true;
    Str json = StrInitFromZstr(
        "{\"status\": true, \"message\": \"Success\", \"data\": {\"12345\": {\"67890\": {\"distance\": 0.85, \"nearest_neighbor_analysis_id\": 999, \"nearest_neighbor_binary_id\": 888, \"nearest_neighbor_analysis_name\": \"test_analysis\", \"nearest_neighbor_function_name\": \"main_func\", \"nearest_neighbor_sha_256_hash\": \"abc123\", \"nearest_neighbor_debug\": true, \"nearest_neighbor_function_name_mangled\": \"_Z4main\"}}}}"
    );
    StrIter si = StrIterFromStr(json);
    ...
    if (response.data.length > 0) {
        ...
        if (StrCmpCstr(&sym->function_name, "main_func", 9) != 0) {
            printf("[DEBUG] Function name check failed: expected 'main_func', got string of length %zu\n", sym->function_name.length);
            success = false;
        }
...
bool test_search_results_with_tags(void) {
    ...
    bool success = true;
    Str json = StrInitFromZstr(
        "{\"binary_id\": 888, \"binary_name\": \"test_binary\", \"analysis_id\": 999, \"sha256\": \"abc123\", \"created_at\": \"2024-04-01\", \"model_id\": 12345, \"model_name\": \"test_model\", \"owned_by\": \"user1\"}"
    );
    StrIter si = StrIterFromStr(json);
...
bool test_conditional_parsing(void) {
    ...
    bool success = true;
    Str json = StrInitFromZstr("{\"status\": true, \"message\": \"Success\", \"data\": {\"12345\": {\"67890\": {\"distance\": 0.85, \"nearest_neighbor_analysis_id\": 999, \"nearest_neighbor_binary_id\": 888, \"nearest_neighbor_analysis_name\": \"test_analysis\", \"nearest_neighbor_function_name\": \"main_func\", \"nearest_neighbor_sha_256_hash\": \"abc123\", \"nearest_neighbor_debug\": true, \"nearest_neighbor_function_name_mangled\": \"_Z4main\"}}}}");
    StrIter si = StrIterFromStr(json);
        ...
        if (response.status) {
            ...
            JR_OBJ_KV(si, "data", {
                ...
                JR_OBJ(si, {
                    ...
                    printf("[DEBUG] Parsed symbol - distance: %f, analysis_id: %llu, binary_id: %llu\n", 
                           sym.distance, sym.analysis_id, sym.binary_id);
