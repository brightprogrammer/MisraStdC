...
bool test_empty_array_writing(void) {
    ...
    JW_OBJ(json, {
        JW_ARR_KV(json, "numbers", empty_numbers, num, {
            JW_INT(json, num);
        });
        JW_ARR_KV(json, "strings", empty_strings, str, {
            JW_STR(json, str);
        });
    });
...
bool test_nested_empty_containers_writing(void) {
    ...
    JW_OBJ(json, {
        JW_OBJ_KV(json, "outer", {
            // Empty nested object
        });
        JW_ARR_KV(json, "list", empty_list, item, {
            JW_INT(json, item);
        });
        JW_OBJ_KV(json, "deep", {
            JW_OBJ_KV(json, "inner", {
...
bool test_mixed_empty_and_filled_writing(void) {
    ...
    JW_OBJ(json, {
        JW_OBJ_KV(json, "empty_obj", {
            // Empty object
        });
        JW_OBJ_KV(json, "filled_obj", {
            JW_INT_KV(json, "x", x_value);
        });
        JW_ARR_KV(json, "empty_arr", empty_arr, item, {
            JW_INT(json, item);
        });
        JW_ARR_KV(json, "filled_arr", filled_arr, item, {
            JW_INT(json, item);
        });
    });
...
bool test_single_values_writing(void) {
    ...
    i32 single_int = 42;
    JW_OBJ(json1, {
        JW_INT_KV(json1, "value", single_int);
    });
    
    // Single string
    Str single_str = StrInitFromZstr("hello");
    JW_OBJ(json2, {
        JW_STR_KV(json2, "text", single_str);
    });
    
    // Single boolean
    bool single_bool = true;
    JW_OBJ(json3, {
        JW_BOOL_KV(json3, "flag", single_bool);
    });
    
    // Single float
    f64 single_float = 3.14;
    JW_OBJ(json4, {
        JW_FLT_KV(json4, "pi", single_float);
    });
    ...
    if (!compare_json_output(&json1, expected1)) success = false;
    if (!compare_json_output(&json2, expected2)) success = false;
    if (!compare_json_output(&json3, expected3)) success = false;
    if (!compare_json_output(&json4, expected4)) success = false;
