...
bool compare_configs(const TestConfig* a, const TestConfig* b) {
    if (a->debug_mode != b->debug_mode || 
        a->timeout != b->timeout ||
        StrCmp(&a->log_level, &b->log_level) != 0 ||
...
bool test_simple_roundtrip(void) {
    ...
    // Compare values
    if (original.count == parsed.count &&
        original.temperature == parsed.temperature &&
        original.enabled == parsed.enabled &&
        StrCmp(&original.message, &parsed.message) == 0) {
        printf("[DEBUG] Simple round-trip test passed\n");
    } else {
        printf("[DEBUG] Simple round-trip test FAILED\n");
        printf("[DEBUG] Original: count=%d, temp=%f, enabled=%s, msg='%s'\n", 
               original.count, original.temperature, original.enabled ? "true" : "false", original.message.data);
        printf("[DEBUG] Parsed: count=%d, temp=%f, enabled=%s, msg='%s'\n", 
               parsed.count, parsed.temperature, parsed.enabled ? "true" : "false", parsed.message.data);
        success = false;
    }
...
bool test_numeric_roundtrip(void) {
    ...
    bool ints_match = (original.big_int == parsed.big_int &&
                      original.negative_int == parsed.negative_int &&
                      original.zero_int == parsed.zero_int);
    
    bool floats_match = (fabs(original.precise_float - parsed.precise_float) < 0.000001 &&
...
bool test_boolean_roundtrip(void) {
    ...
    // Compare values
    if (original.flag1 == parsed.flag1 &&
        original.flag2 == parsed.flag2 &&
        original.flag3 == parsed.flag3 &&
...
bool test_string_roundtrip(void) {
    ...
    // Compare values
    if (parsed.empty.length == original.empty.length &&
        StrCmp(&original.simple, &parsed.simple) == 0 &&
...
bool test_array_roundtrip(void) {
    ...
    Str json = StrInit();
    JW_OBJ(json, {
        JW_ARR_KV(json, "numbers", original_numbers, num, {
            JW_INT(json, num);
        });
        JW_ARR_KV(json, "strings", original_strings, str, {
            JW_STR(json, str);
        });
    });
    ...
    if (strings_match) {
        for (size i = 0; i < VecLen(&original_strings); i++) {
            if (VecAt(&original_strings, i).length != VecAt(&parsed_strings, i).length ||
                (VecAt(&original_strings, i).length && StrCmp(&VecAt(&original_strings, i), &VecAt(&parsed_strings, i)) != 0)) {
                strings_match = false;
                break;
            }
        }
    ...
    if (numbers_match && strings_match) {
        ...
        printf("[DEBUG] Numbers match: %s (orig %zu, parsed %zu)\n", 
               numbers_match ? "true" : "false", VecLen(&original_numbers), VecLen(&parsed_numbers));
        printf("[DEBUG] Strings match: %s (orig %zu, parsed %zu)\n", 
               strings_match ? "true" : "false", VecLen(&original_strings), VecLen(&parsed_strings));
        success = false;
    }
...
bool test_nested_object_roundtrip(void) {
    ...
    // Original data
    TestPerson original_person = {
        12345,
        StrInitFromZstr("John Doe"),
        30,
        true,
        75000.50
    };
...
bool test_complex_data_roundtrip(void) {
    ...
    JW_OBJ(json, {
        ...
        JW_OBJ_KV(json, "config", {
            ...
            JW_ARR_KV(json, "features", original.config.features, feature, {
                JW_STR(json, feature);
            });
        });
        JW_ARR_KV(json, "numbers", original.numbers, num, {
            JW_INT(json, num);
        });
        JW_ARR_KV(json, "flags", original.flags, flag, {
            JW_BOOL(json, flag);
        });
    });
...
bool test_empty_containers_roundtrip(void) {
    ...
    JW_OBJ(json, {
        ...
        JW_ARR_KV(json, "empty_numbers", empty_numbers, num, {
            JW_INT(json, num);
        });
        JW_ARR_KV(json, "empty_strings", empty_strings, str, {
            JW_STR(json, str);
        });
        JW_OBJ_KV(json, "empty_object", {
            // Empty nested object
        });
    });
    ...
    JR_OBJ(si, {
        ...
        JR_OBJ_KV(si, "empty_object", {
            found_empty_object = true;
        });
    });
    
    // Compare empty containers
    if (parsed_str.length == 0 &&
        VecLen(&parsed_numbers) == 0 &&
        VecLen(&parsed_strings) == 0 &&
        ...
        printf("[DEBUG] String length: %zu, numbers: %zu, strings: %zu, found_obj: %s\n",
               parsed_str.length, VecLen(&parsed_numbers), VecLen(&parsed_strings), 
...
bool test_edge_cases_roundtrip(void) {
    ...
    // Compare edge cases
    if (original.max_int == parsed.max_int &&
        original.min_int == parsed.min_int &&
        original.zero == parsed.zero &&
        original.zero_float == parsed.zero_float &&
        original.true_val == parsed.true_val &&
