...
bool test_two_level_nesting_writing(void) {
    ...
    struct {
        ...
        Str status;
    } data = {{123, {StrInitFromZstr("Alice"), 30}}, StrInitFromZstr("active")};
...
bool test_three_level_nesting_writing(void) {
    ...
    } data = {{{{StrInitFromZstr("John"), 25, 150000.0}}, StrInitFromZstr("TechCorp")}};
    ...
    const char* expected = "{\"company\":{\"departments\":{\"engineering\":{\"head\":\"John\",\"count\":25,\"budget\":150000.000000}},\"name\":\"TechCorp\"}}";
    if (!compare_json_output(&json, expected)) {
        success = false;
    }
...
bool test_function_info_array_writing(void) {
    ...
    const char* expected = "{\"functions\":[{\"id\":12345,\"name\":\"test_func\",\"size\":1024,\"vaddr\":4096},{\"id\":54321,\"name\":\"helper_func\",\"size\":512,\"vaddr\":8192}]}";
    if (!compare_json_output(&json, expected)) {
        success = false;
    }
...
bool test_search_results_with_tags_writing(void) {
    ...
    JW_OBJ(json, {
        ...
        JW_ARR_KV(json, "tags", result.tags, tag, {
            JW_STR(json, tag);
        });
    ...
    const char* expected = "{\"binary_id\":888,\"binary_name\":\"test_binary\",\"analysis_id\":999,\"sha256\":\"abc123\",\"tags\":[\"malware\",\"x86\"],\"created_at\":\"2024-04-01\",\"model_id\":12345,\"model_name\":\"test_model\",\"owned_by\":\"user1\"}";
    if (!compare_json_output(&json, expected)) {
        success = false;
    }
...
bool test_dynamic_object_keys_writing(void) {
    ...
    const char* expected = "{\"functions\":{\"111\":{\"222\":{\"distance\":0.900000,\"name\":\"func1\"}},\"333\":{\"444\":{\"distance\":0.800000,\"name\":\"func2\"}}}}";
    if (!compare_json_output(&json, expected)) {
        success = false;
    }
...
bool test_deeply_nested_structure_writing(void) {
    ...
    const char* expected = "{\"level1\":{\"level2\":{\"level3\":{\"message\":\"deep\",\"value\":42},\"flag\":true},\"name\":\"test\"}}";
    if (!compare_json_output(&json, expected)) {
        success = false;
    }
...
bool test_mixed_array_types_writing(void) {
    ...
    JW_OBJ(json, {
        JW_ARR_KV(json, "numbers", numbers, num, {
            JW_INT(json, num);
        });
        JW_ARR_KV(json, "strings", strings, str, {
            JW_STR(json, str);
        });
        JW_ARR_KV(json, "booleans", booleans, b, {
            JW_BOOL(json, b);
        });
    });
