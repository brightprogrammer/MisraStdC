...
bool test_vec_foreach_ptr_in_range_idx_out_of_bounds_access(void) {
    ...
    VecForeachPtrInRangeIdx(&vec, val_ptr, idx, 0, original_length) {
        ...
        if (idx == 3) {
            ...
            WriteFmt(
                "Deleted first 6 elements, new length={}, but range ptr iteration continues to idx {}...\n",
                vec.length,
                original_length
            );
        }
...
bool test_vec_foreach_idx_basic_out_of_bounds_access(void) {
    ...
    VecForeachIdx(&vec, val, idx) {
        ...
        if (idx == 2) {
            ...
            WriteFmt("Vector resized to length {}, but basic foreach iteration continues...\n", vec.length);
        }
...
int main(void) {
    WriteFmt("[INFO] Starting Vec.Foreach.Deadend tests\n\n");
