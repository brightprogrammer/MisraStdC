...
// ===========================================================================
// Merged mutation-kill fixtures + tests from Dwarf.Mut.
...
//     *Deinit on a reachable error branch becomes a detectable leak.
// The shared put_u16/put_u32/put_u64, kStdOpcodeLengths, emit_set_address8,
// emit_end_sequence, and build_elf_with_debug_line_8192 helpers above are
// reused; only the additional opcodes, header constants, fixture struct and
// fixture builders below are specific to these tests.
// ===========================================================================

// Extra standard opcodes used by these fixtures (the shared five -- COPY,
// ADVANCE_PC, ADVANCE_LINE, END_SEQUENCE, SET_ADDRESS -- are defined above).
enum {
    DWLNS_SET_FILE         = 0x04,
...
typedef struct DwmLineFixture {
    u8 *unit_len_field;
} DwmLineFixture;

static u8 *dwm_write_line_header(u8 *buf, DwmLineFixture *fx) {
    u8 *p               = buf;
    fx->unit_len_field  = p;
    p                  += 4; // unit_length (filled later)
    put_u16(p, 4);
...
static u64 dwm_finalize_unit(const DwmLineFixture *fx, u8 *buf, u8 *p) {
    put_u32(fx->unit_len_field, (u32)(p - (fx->unit_len_field + 4)));
    return (u64)(p - buf);
}
...
bool test_dwm_file_off_init_zero_when_file_oob(void) {
    ...
    u8             dl[512];
    DwmLineFixture fx;
    u8            *p = dwm_write_line_header(dl, &fx);
    *p++             = DWLNS_SET_FILE;
