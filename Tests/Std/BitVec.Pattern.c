...
bool test_bitvec_find_all_pattern_null_source(void) {
    ...
    BitVecFindAllPattern(NULL, &pattern, results, 10); // Should cause LOG_FATAL
...
bool test_bitvec_replace_null_source(void) {
    ...
    BitVec old_pat = BitVecInit();
    BitVec new_pat = BitVecInit();
    BitVecPush(&old_pat, true);
    BitVecPush(&new_pat, false);
    BitVecReplace(NULL, &old_pat, &new_pat);
    BitVecDeinit(&old_pat);
    BitVecDeinit(&new_pat);
    return true;
}
