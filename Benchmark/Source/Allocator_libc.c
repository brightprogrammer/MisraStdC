...
uint64_t bench_live_bytes(void) {
    ...
    // own API. The bench harness has no fallback -- if a backend
    // returns 0 here, the fragmentation table reports n/a for live
    // for that backend rather than synthesising a value.
#if BENCH_BACKEND_GLIBC
    ...
    // counter is the closest but is what bench_footprint_bytes
    // returns. No reliable live counter -- report 0 so the
    // fragmentation table reads n/a for mimalloc live.
    return 0;
#elif BENCH_BACKEND_TCMALLOC
    size_t v = 0;
    if (!MallocExtension_GetNumericProperty("generic.current_allocated_bytes", &v))
        return 0;
    return (uint64_t)v;
#else
    return 0;
...
uint64_t bench_footprint_bytes(void) {
    ...
    size_t v = 0;
    if (!MallocExtension_GetNumericProperty("generic.heap_size", &v))
        return 0;
    return (uint64_t)v;
#else
    return 0;
