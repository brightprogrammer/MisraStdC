...
`HeapAllocator` and `SlabAllocator` benchmarked against glibc, jemalloc, mimalloc, and tcmalloc.

Full results, methodology, and reproduction steps live in **[Benchmark/README.md](Benchmark/README.md)** — regenerated by `Benchmark/Scripts/run.py` against the build directory, so the numbers there are always for the host that last ran the script.

To rebuild the numbers yourself:
